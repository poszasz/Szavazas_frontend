...
export default function MyCardsPage() {
    ...
    useEffect(() => {
        const verifyAuth = async () => {
            console.log("MyCardsPage: Checking authentication...")
            const { authenticated, user } = await checkAuth()
            console.log("MyCardsPage: Auth result:", authenticated, user)
            
            if (!authenticated) {
                console.log("MyCardsPage: Not authenticated, redirecting to login")
                navigation('/login')
                return
            }
            
            console.log("MyCardsPage: Authenticated as:", user)
            setUser(user)
            await loadCards()
            setLoading(false)
        }
    ...
    const loadCards = async () => {
        console.log("MyCardsPage: Loading cards...")
        setLoading(true)
        const res = await getMyCards()
        console.log("MyCardsPage: Cards loaded:", res)
