...
export default function MarketPage() {
    ...
    useEffect(() => {
        const verifyAuth = async () => {
            console.log("MarketPage: Checking authentication...")
            const { authenticated, user } = await checkAuth()
            console.log("MarketPage: Auth result:", authenticated, user)
            
            if (!authenticated) {
                console.log("MarketPage: Not authenticated, redirecting to login")
                navigation('/login')
                return
            }
            
            console.log("MarketPage: Authenticated as:", user)
            setUser(user)
            await Promise.all([
                loadListings(),
                loadMyCards(),
    ...
    const loadMyPendingOffers = async () => {
        console.log("MarketPage: Loading pending offers...")
    ...
    const loadListings = async () => {
        console.log("MarketPage: Loading listings...")
        const res = await getMarketListings()
        if (res.result) {
            console.log("Listings loaded:", res.listings.length)
            setListings(res.listings)
        }
    }

    const loadMyCards = async () => {
    console.log("MarketPage: Loading my cards...")
    const res = await getMyCards()
    if (res.result) {
        console.log("Kártyák a backendből:", res.cards)
        
        const availableCards = res.cards.filter(
            card => !card.is_listed && !card.is_offered
        ).map(card => ({
            ...card,
            user_card_id: card.user_card_id  // Ez most a user_cards.id (1,2,3,4)
        }))
        
        console.log("Elérhető kártyák:", availableCards)
    ...
    const handleCreateListing = async () => {
        ...
        console.log("Kiválasztott kártya:", selectedCardForListing.manufacturer, selectedCardForListing.name)
    ...
    const handleOfferSubmit = async () => {
        ...
        console.log("Selected User Card ID:", selectedUserCardId)
        console.log("Selected Card:", selectedCard?.manufacturer, selectedCard?.name)
        console.log("=========================")
